 ...
 ```sh
 cargo run --bin ga -- --hpo-term HP:0001083 -p 20 -g 5 -m 0.5 -t 3 --max-n-terms 5 --max-n-conj 4 --rng-seed 42
 ```
 ...
 ```sh
 cargo run --bin ga_batch -- src/bin/run_configs/hpo_focused_one_run.ljson \
   --summary-file stats/runs/summaries/hpo_focused_one_run.txt \
 ...
 - `--summary-file, -s <path>` optional text table (relative paths are placed under `stats/runs/summaries/`).
