    ...
    pub fn map_keys<T>(&self, hashmap: HashMap<String, T>) -> HashMap<GeneId, T> {
        ...
        for (old_gene_id, content) in hashmap {
            match self.map(&old_gene_id) {
                Some(new_gene_id) => {
                    new_hashmap.insert(new_gene_id, content);
                }
                None => {
                    n_unmapped += 1;
                }
        ...
        if n_unmapped > 0 {
            println!(
                "A total of {} terms haven't been mapped successfully",
                n_unmapped
            )
        }
...
mod tests {
    ...
    fn test_gene_id_mapper() -> io::Result<()> {
        ...
        let tissue_expressions =
            remove_ensembl_id_version_number(GtexSummaryParser::parse(&summary));
        ...
        assert!(
            !mapped_tissue_expressions.is_empty(),
