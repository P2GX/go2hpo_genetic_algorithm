...
impl GtexSummaryParser{
    pub fn parse(summary: &GtexSummary) -> HashMap<String, HashSet<TissueExpression>>{
        ...
        for (gene, result) in results{
            ...
            let mut tissue_expr_set: HashSet<TissueExpression> = result.up_regulated.iter()
                    .map(|tissue_analysis| TissueExpression::new(tissue_analysis.tissue_name.clone(),DgeState::Up ))
                    .collect();

            tissue_expr_set.extend(
                result.down_regulated.iter().map(|tissue_analysis| 
    ...
    fn test_gtex_parse() -> io::Result<()>{
        ...
        for (gene, result) in results{
            ...
            if result.down_regulated.len() > 0{
                ...
                let tissue_expr_down = TissueExpression::new(tissue_down.to_string(), DgeState::Down);
