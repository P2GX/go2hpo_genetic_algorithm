...
use crate::{
    annotations::{GeneId, GeneSetAnnotations},
    logical_formula::{Conjunction, SatisfactionChecker, DNF},
};
...
pub trait FitnessScorer<T, P> {
    ...
    fn custom_score_fitness(
        &self,
        formula: &T,
        phenotype: &TermId,
        custom_score_metric: &ScoreMetric,
    ) -> f64;
}
...
impl<C: SatisfactionChecker, T: DNF> FitnessScorer<T, TermId> for DNFScorer<C> {
    fn fitness(&self, formula: &T, phenotype: &TermId) -> f64 {
        ...
        self._fitness(
            formula,
            phenotype,
            self.conjunction_scorer.get_metric(),
            self.penalty_lambda,
        )
    }
...
impl<'a, C: SatisfactionChecker> DNFScorer<C> {
    ...
    fn _fitness<T: DNF>(
        ...
        let base_score = match custom_score_metric {
            ScoreMetric::Accuracy => self
                .conjunction_scorer
                .accuracy(&genes_satisfaction, phenotype),
            ScoreMetric::Precision => self
                .conjunction_scorer
                .precision(&genes_satisfaction, phenotype),
            ScoreMetric::Recall => self
                .conjunction_scorer
                .recall(&genes_satisfaction, phenotype),
            ScoreMetric::FScore(beta) => self
                .conjunction_scorer
                .fs_score(&genes_satisfaction, phenotype),
        };
...
impl<'a, C: SatisfactionChecker> DNFScorer<C> {
    pub fn get_genes_satisfaction_for_dnf<T: DNF>(
        &self,
        formula: &T,
    ) -> anyhow::Result<HashMap<GeneId, bool>> {
        let conjunctions = formula.get_active_conjunctions();
        if conjunctions.len() == 0 {
            anyhow::bail!("The conjunction length shouldn't be 0")
        }

        let mut conjunction_iter = conjunctions.iter();
        let first_conjunction = conjunction_iter
            .next()
            .expect("It should have at least the first element");
        let mut satisfaction_mask = self
            .conjunction_scorer
            .checker
            .all_satisfactions(first_conjunction);
...
impl<C: SatisfactionChecker> ConjunctionScorer<C> {
    ...
    pub fn accuracy(&self, genes_satisfaction: &HashMap<GeneId, bool>, phenotype: &TermId) -> f64 {
        ...
        let correct_predictions = self
            ...
            .iter()
            .filter(|(gene, gene_annotation)| {
                gene_annotation.contains_phenotype(phenotype)
                    == *genes_satisfaction
                        .get(*gene)
                        .expect("There should be an entry for the gene")
            })
            .count();
    ...
    pub fn precision(&self, genes_satisfaction: &HashMap<GeneId, bool>, phenotype: &TermId) -> f64 {
        ...
        for (gene, gene_annotation) in self.checker.get_gene_set().get_gene_annotations_map() {
            let predicted = *genes_satisfaction
                .get(gene)
                .expect("Missing prediction for gene");
...
mod tests {
    ...
    use crate::{
        ...
        logical_formula::{DNFVec, DgeState, TermObservation, TissueExpression},
    };
        ...
        fn get_gene_set(&self) -> &GeneSetAnnotations {
            if self.gene_set_option == 1 {
                return Box::leak(Box::new(GeneSetAnnotations::new({
                    ...
                    map.insert(
                        "gene1".into(),
                        make_gene_annotations("gene1".into(), &["HPO:0000001"], &[]),
                    );

                    // gene2: FP
                    map.insert(
                        "gene2".into(),
                        make_gene_annotations("gene2".into(), &[], &[]),
                    );

                    // gene3: FN
                    map.insert(
                        "gene3".into(),
                        make_gene_annotations("gene3".into(), &["HPO:0000001"], &[]),
                    );

                    // gene4: TN
                    map.insert(
                        "gene4".into(),
                        make_gene_annotations("gene4".into(), &[], &[]),
                    );
                ...
                return Box::leak(Box::new(GeneSetAnnotations::new({
                    ...
                    map.insert(
                        ...
                        make_gene_annotations(
                            "gene1".into(),
                            &["HPO:0000001", "HPO:0000002", "HPO:0000003"],
                            &[],
                        ),
                    );
                    map.insert(
                        "gene2".to_string(),
                        make_gene_annotations("gene2".into(), &["HPO:0000002", "HPO:0000003"], &[]),
                    );
                    map.insert(
                        "gene3".to_string(),
                        make_gene_annotations("gene3".into(), &["HPO:0000001", "HPO:0000003"], &[]),
                    );
                    map.insert(
                        "gene4".to_string(),
                        make_gene_annotations("gene4".into(), &[], &[]),
    ...
    fn test_display_solution() {
        ...
        let conj = Conjunction {
            term_observations: vec![TermObservation::new(t1, false)],
